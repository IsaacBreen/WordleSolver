...
public:
    ...
    double information_gain(const string &guess, const tuple<map<char, map<int, bool>>, set<char>> &condition = make_tuple(map<char, map<int, bool>>(), set<char>()))
    {
    ...
    double expected_information_gain(const tuple<map<char, map<int, bool>>, set<char>> &condition = make_tuple(map<char, map<int, bool>>(), set<char>()))
    {
        double expected_information_gain = 0;
        for (const string &guess : valid_guesses)
        {
            expected_information_gain += information_gain(guess, condition);
        }
        return expected_information_gain / valid_guesses.size();
    }
    ...
    tuple<string, string, double, double> find_optimal_guesses(const tuple<map<char, map<int, bool>>, set<char>> &condition)
    {
        ...
        for (const string &guess : valid_guesses)
        {
            double this_information_gain = information_gain(guess, condition);
    ...
    void apply_guess_and_result(const string &guess, const string &result)
    {
        auto new_condition = convert_pretty_condition_to_condition(guess, result);
        apply_condition(new_condition);
    }
};
...
int main()
{
    ...
    // Get expected information gain for 'soare'
    cout << "Expected information gain for 'soare' is " << wordle.information_gain("soare", make_tuple(map<char, map<int, bool>>{}, set<char>{})) << endl;
...
// Compile with:
// g++ -std=c++17 -O3 -o wordle wordle.cpp && ./wordle